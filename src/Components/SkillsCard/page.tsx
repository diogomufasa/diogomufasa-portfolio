...
const SkillsCard = ({ title, items }: SkillsCardProps) => {
  ...
  return (
    <div className='skill_card h-fit w-full sm:w-[47%]  p-3 rounded-md shadow-inner dark:bg-zinc-900 dark:shadow-black mb-2 animateOff g-transition'>
      <div
        className='flex-between cursor-pointer'
        onClick={handleOpenUiUxBoxOpen}
      >
        <h4>{title}</h4>
        <BsChevronDown />
      </div>
